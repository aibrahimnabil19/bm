...
export default function RetirerArgentModal({ isOpen, onClose, onSaved, banque }) {
  ...
  useEffect(() => {
    if (!isOpen) return;
    if (sousType === "paiement_sonidep") {
      setLoadingFactures(true);
      supabase.from("factures")
        .select("*")
        .eq("statut", "en_attente")
        .order("periode_debut", { ascending: false })
        .then(({ data }) => { setFactures(data ?? []); setLoadingFactures(false); });
    }
  }, [isOpen, sousType]);

  if (!isOpen) return null;

  // If user clicks "Payer" on a facture → hand off to PayerFactureModal
  if (payingFacture) {
    return (
      <PayerFactureModal
        isOpen={true}
        ...
        onSaved={() => { setPayingFacture(null); onClose(); onSaved?.(); }}
      />
    );
  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="bg-white rounded-xl shadow-lg w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200 max-h-[90vh] flex flex-col">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4 overflow-y-auto">
            ...
            <div className="flex flex-col gap-2">
              {SOUS_TYPES.map((s) => (
                <button key={s.value} type="button"
                  onClick={() => { setSousType(s.value); setSelectedFacture(null); setFormData({ montant: "", reference: "", description: "" }); }}
          ...
          {/* ── SONIDEP: show facture list ── */}
          {sousType === "paiement_sonidep" && (
            <div className="animate-in fade-in duration-200">
              <p className="text-sm font-medium text-slate-700 mb-2">Factures en attente</p>
              {loadingFactures ? (
                <p className="text-sm text-slate-400">Chargement...</p>
              ) : factures.length === 0 ? (
                <div className="rounded-lg border-2 border-dashed border-slate-200 p-4 text-center">
                  <p className="text-sm text-slate-400">Aucune facture en attente.</p>
                </div>
              ) : (
                <div className="space-y-2">
                  {factures.map((f) => (
                    <div key={f.id}
                      className="flex items-center justify-between p-3 rounded-lg border border-slate-200 bg-slate-50 hover:bg-orange-50 hover:border-orange-200 transition"
                    >
                      <div>
              ...
              {/* No submit form for this path — PayerFactureModal handles it */}
                ...
                {imagePreview && (
                  <div className="relative mt-2">
                    ...
                    <button type="button"
                      ...
                      className="absolute top-1 right-1 bg-white rounded-full w-6 h-6 flex items-center justify-center shadow text-slate-500 hover:text-red-500 text-xs">✕</button>
                  </div>
                )}
              </div>
